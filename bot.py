...
LORE_TEXT = (
    "🚨 СРОЧНО! СКАНДАЛ!\n\n"
    "«РОНАЛДУ ПРОПАЛ В ПИВНОМ ЛАБИРИНТЕ»\n"
    "— наш спецрепортаж\n\n"
    "Лето 2018-го. После финала ЧМ Криштиану "
    "Роналду решил отметить по-русски и заглянул "
    "в первую попавшуюся пивную — «У БАБЫ КЛАВЫ».\n\n"
    "«О, вроде нормальное место», — успел подумать "
    "он. 🍺 Внутри уже ждали четверо: Маша, Сергей, "
    "Лена и Сальвини. «Заходи, заходи...»\n\n"
    "Сначала всё было хорошо. Пили, смотрели повтор "
    "матча, обнимались. Но на пятой кружке Сергей "
    "вдруг сказал:\n\n"
    "— Криш, а давай ты теперь за сборную России "
    "играть будешь? Ну а чо? Паспорт сделаем.\n\n"
    "Криштиану молча встал и пошёл к выходу. Но "
    "выхода не было. ЗАЛ 2. ЗАЛ 3. Те же бочки, те "
    "же указатели «ВЫХОД» в никуда. А сзади уже "
    "топот — четверо бегут с криками:\n\n"
    "— КРИШ, ПОДУМАЙ! У НАС ХОРОШО!\n\n"
    "🏃 Беги, Криш. Беги. 👇"
)
...
def play_button() -> ReplyKeyboardMarkup:
    """Reply-keyboard button that opens the Mini App.

    IMPORTANT: We use ReplyKeyboardMarkup (not InlineKeyboardMarkup) because
    `tg.sendData()` from the Mini App only works when the app was launched
    via a KeyboardButton — Telegram disallows sendData for inline/menu buttons.

    Layout (B): "Играть" full-width on top, "Лор" + "Топ" side-by-side below.
    """
    return ReplyKeyboardMarkup(
        [
            [KeyboardButton(text="🎮 Играть", web_app=WebAppInfo(url=WEB_APP_URL))],
            [KeyboardButton(text="📖 Лор"), KeyboardButton(text="🏆 Топ")],
        ],
...
async def fallback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Any non-command text message → check if it's a reply-keyboard tap.

    Reply-keyboard buttons send their text label as a regular message.
    We map known labels to the same handlers as their slash commands.
    """
